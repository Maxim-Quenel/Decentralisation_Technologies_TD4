...
export async function rsaEncrypt(
  ...
  const dataArrayBuffer = base64ToArrayBuffer(b64Data);
...
export async function rsaDecrypt(
  ...
  const dataArrayBuffer = base64ToArrayBuffer(data);
...
export async function symEncrypt(
  ...
  // Concatenate IV and encryptedData, and convert to base64
...
export async function symDecrypt(
  ...
  // Split IV (first 16 bytes) and encrypted content
  const combinedArray = new Uint8Array(combinedArrayBuffer);
  const iv = combinedArray.slice(0, 16); // Take the first 16 bytes as the IV
  const data = combinedArray.slice(16); // Rest is the encrypted data
